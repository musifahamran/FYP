...
class Cnn10(nn.Module):
    def __init__(self,num_classes=4, in_ch=3, pretrained=True):
        ...
        self.conv_block1 = ConvBlock(in_channels=in_ch, out_channels=64)
    ...
    def init_weight(self,pretrained=True):
        init_bn(self.bn0)
        init_layer(self.fc1)
        init_layer(self.fc)
        # if(pretrained==True):
        #     self.load_state_dict(torch.load('/home/FYP/musi0002/speech_emo_recognition/Cnn10_mAP=0.380.pth')['model'], strict=False)

    def forward(self, input):

        x = input.unsqueeze(3)   # (batch_size, 1, time_steps, mel_bins)
        x = x.transpose(1, 3)
        x = self.bn0(x)
        x = x.transpose(1, 3)
...
class Cnn14(nn.Module):
    ...
    def init_weight(self, pretrained=True):
        ...
        # if (pretrained == True):
        #     self.load_state_dict(torch.load('/home/FYP/musi0002/speech_emo_recognition/Cnn14_mAP=0.431.pth')['model'], strict=False)
