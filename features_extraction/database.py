...
class IEMOCAP_8k_database():
    """
    The same IEMOCAP database however in 8kHz sampling rate.
    ...
    def get_files(self):
        ...
        for session_name in os.listdir(dataset_dir):
            ...
            for conversation_folder in os.listdir(wav_dir):
                ...
                # Path to the directory containing all the *.wav files of the
                # current conversation -> for 8k, os.path.join(wav_dir, conversation_folder,"8k")
                ...
                # print(wav_files)
                # separate into individual speakers
                F_wav.extend([emowav for emowav in wav_files if emowav[0][68] == "F"])

                # -8 for Original IEMOCAP, 68,69 for 8k
                M_wav.extend([emowav for emowav in wav_files if emowav[0][68] == "M" | emowav[0][69] == "M"])
...
class THAI_database():
    """
   THAI database contains data from 200 actors, 88 male and 112 female. The database consists of
    80 studio and 20 Zoom recordings, containing both improvised and scripted sessions.
    Each of files has a unique filename, provided in .flac format with sample rate about 44.1 KHz.
    The filename consists of a 5 to 6-part identifier. For this feature extraction process, the files have been
    downsampled to 16kHz and converted to .wav format.

    The recordings are split into batches of 10. For example
        eg. Studio001-010
                |-- Studio001
                |-- Studio002
                |-- ...
    In each studio, they are further split into microphone type used
        e.g. Studio001
                |--_con
                |--clip
                |--middle

    In each mic folder, the wav files are named
        e.g. Studio001
                |--_con
                    |-- s001_clip_actor002_impro2_32 -> RecordingID_micTpye_actorID_SessionID(improvisation/scripted)

    This function extract utterance filenames and labels for improvised sessions,
    organized into dictionary of {'speakerID':[(conversation_wavs,lab),(wavs,lab),...,(wavs,lab)]}

        > speakerID eg. 1T: Studio 1-5, 1V: Studio 6-10 ...

    Database Reference:
        (2021). THAI Speech emotion recognition dataset.
    ...
    def __init__(self, database_dir, emot_map= {'Angry': 0, 'Sad': 1, 'Happy': 2, 'Neutral': 3},
        ...
        # IEMOCAP available emotion classes
        self.all_emo_classes = THAI_EMO_CODES.keys()
    ...
    def get_files(self):
        ...
        for studio_batches in os.listdir(dataset_dir):
            ...
            for studio_folder in os.listdir(wav_dir):
                ...
                # separate into Test and Dev set
                if count <= 5:
